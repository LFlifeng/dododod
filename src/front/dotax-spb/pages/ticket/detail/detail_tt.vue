	...
	<view>
		...
		<view class="head-section">
			<picker v-if="edit" @change="handleInvoiceTypeChange" :value="invoiceTypePickerValue" :range="invoiceTypes"
			 range-key="label">
				<text>{{fmtInvoiceType(invoice.invoiceType)}}<image class="right_icon" src="../../../static/Chevron.png" mode=""></image></text>
			</picker>
			<text v-if="!edit">{{fmtInvoiceType(invoice.invoiceType)}}</image></text>
		...
		<view :class="{'oprt-section-wt':!invoiceImageVisible}">
		...
		<view class="oprt-section" >
			<view class="btn-group">
				<button v-if="deleteButtonVisible" type="default" class="s_btn" @click="handleDelete">删除</button>
...
<style lang="scss">
	...
	.uni-collapse-cell--hide.uni-collapse-cell {
		background-color: #f4f4f4;
		border: none;
		text-align: center;
	}
