	...
	<view class="ri-main">
		<view class="head-section">
			<view class="head-content">
				...
				<view>识别成功:<text>{{successCount}}</text>张</view>
		...
		<view class="foot-section">
			<view class="foot-content">
				<button @click="handleSubmit" v-bind:disabled="totalCount==0" type="primary">全部提交</button>
			</view>
		</view>
	</view>
</template>
			...
			handleDetail(gidx, iidx) {
				...
				uni.setStorage({
					...
					success: () => {
						...
						uni.navigateTo({
							url: `../detail/detail?biz=${that.biz}&edit=true&state=${state}`
						});
					},
					fail: () => {
						uni.showToast({
							...
							if (that.biz == 'dksp') {
								deductionPushInvoices(invoices).then((piRes) => {
									...
									if (piRes && piRes.success) {
										...
										uni.redirectTo({
											url: '/pages/income/submitPage/submitPage?invCount=' + invoices.length
										});
									}
								});
							} else {
